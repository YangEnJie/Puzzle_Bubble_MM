...
{	
	...
	/*
	首先将自己isSame设置为true，并且增加sameSum
	...
	*/
	...
	if (flag && j > 0 && i > 0) 	{
		findTheSameBubble(i - 1, j - 1, !flag, type); //左上
	}
	else if (!flag && i > 0) 	{
	...
	if (flag && j > 0 && i < MAX_ROWS) 	{  //左下
		findTheSameBubble(i + 1, j - 1, !flag, type);
	}
	else if (!flag && i < MAX_ROWS) 	{ 
		findTheSameBubble(i + 1, j, !flag, type);
	}
	if (flag && i < MAX_ROWS) 	{  // 右下
		findTheSameBubble(i + 1, j, !flag, type);
	}
...
void BubbleLayer::deleteTheSameBubble(int i, int j, bool flag) //当超过三个掉落时，通过圆环式算法判断范围
{
	if (sameSum < 3)
	{
		for (int i = 0; i < MAX_ROWS; ++i) 	{
	...
	{
		auto gameScene = (GameScene*)(this->getParent()); // 增加掉落的分数
...
void BubbleLayer::checkDownBubble(int i, int j)
{
	...
	if (j > 0)
	{
		checkDownBubble(i, j - 1); // 左
	}
	if (j < MAX_COLS)
	{
		checkDownBubble(i, j + 1); // 右
	}  
}
